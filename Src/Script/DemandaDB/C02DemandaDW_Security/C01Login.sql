...
if not exists (SELECT * FROM master.sys.server_principals where sid = SUSER_SID(N'$(DATABASE_PROXYWINDOWSUSER)'))
BEGIN
 declare @logindb nvarchar(132), @sql nvarchar(2000) 
 select @logindb = N'master'
 if (@logindb IS NULL OR LTrim(@logindb) = '') OR NOT EXISTS (select * from master.dbo.sysdatabases where name = @logindb)
	select @logindb = N'master'
 set @sql = 'CREATE LOGIN [$(DATABASE_PROXYWINDOWSUSER)] FROM WINDOWS WITH DEFAULT_DATABASE=' + @logindb;
 EXECUTE(@sql);
END
GO
